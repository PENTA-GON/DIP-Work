...
counts = ones(k,1); % the number of feature samples belong to each class
...
for M = 1:max_iters
   ...
   for i = 1:size(samples,2)
       min = distance(samples(:,i),aver(:,1),1);%between the i-th sample and the first cluster center
       min_ind = 1; 
       for j = 2:k
           dis = distance(samples(:,i),aver(:,j),1);
           if(dis < min)
               min = dis;
               min_ind = j;
           end
